...
postUserR nick = do
    ...
    case auth of
        True -> do
            ((res, widget),enctype) <- runFormPost $ updatePermissionsMForm peron
            case res of
...
updatePermissionsMForm (Person nick password email info permissions) token = do
    (permissionsResult, permissionsView)  <- mopt (selectFieldList [(("User" :: Text),3), (("Moderator" :: Text), 2), (("Admin" :: Text), 1)]) "" (Just permissions)
